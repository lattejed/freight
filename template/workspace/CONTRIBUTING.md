...
The <%[PROJECT_NAME]%> project adheres to the [Rust Code of Conduct][rust-coc]. This code of conduct describes the _minimum_ behavior expected from all contributors.

Instances of violations of the Code of Conduct can be reported by contacting the team
at [<%[PROJECT_CONTACT_EMAIL]%>](mailto:<%[PROJECT_CONTACT_EMAIL]%>).
...
1. **By opening an issue:** For example, if you believe that you have uncovered a bug
   in <%[PROJECT_NAME]%>, creating a new issue in the issue tracker is the way to report it.
...
- The <%[PROJECT_NAME]%> version you are on (and that it is up to date)
...
Pull requests are the way concrete changes are made to the code, documentation, and dependencies of <%[PROJECT_NAME]%>.
...
If the change being proposed alters code, it is either adding new functionality to <%[PROJECT_CONTACT_EMAIL]%>, or fixing existing, broken functionality. In both of these cases, the pull request should include one or more tests to ensure that <%[PROJECT_CONTACT_EMAIL]%> does not regress in the future.
...
By default, `cargo test` does not select any packages, in order to run individual tests by name (e.g. `cargo test mytest`) navigate to the directory of that file or use the `-p <package-name>` option to run specific tests of a crate from anywhere in the workspace (`cargo test -p <package-name> <test-name>`).
...
**Any <%[PROJECT_NAME]%> community member is welcome to review any pull request**.

All contributors who choose to review and provide feedback on pull requests have a responsibility to both the project and individual making the contribution. Reviews and feedback must be helpful, insightful, and geared towards improving the contribution as opposed to simply blocking it. If there are reasons why you feel the PR should not be merged,
explain what those are. Do not expect to be able to block a PR from advancing simply because you say "no" without giving an explanation. Be open to having your mind changed. Be open to working _with_ the contributor to make the pull request better.
...
1. Does this change make sense for <%[PROJECT_NAME]%>?
2. Does this change make <%[PROJECT_NAME]%> better, even if only incrementally?
...
Nits (requests for small changes that are not essential) are fine, but try to avoid stalling the pull request. Most nits can typically be fixed by the <%[PROJECT_NAME]%> maintainers merging the pull request, but they can also be an opportunity for the contributor to learn a bit more about the project.
...
Be aware that _how_ you communicate requests and reviews in your feedback can have a significant impact on the success of the pull request. Yes, we may merge a particular change that makes <%[PROJECT_NAME]%> better, but the individual might just not want to have anything to do with <%[PROJECT_NAME]%> ever again. The goal is not just having good code.
